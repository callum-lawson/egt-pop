"""PLR training script for T-maze environment.

This script trains an agent on T-maze using Prioritized Level Replay (PLR).
The T-maze has only 2 possible levels (goal on left or right), making it
a minimal test case for PLR.

Based on maze_plr.py with modifications for T-maze:
- Uses TMaze environment with hidden goal
- Level buffer capacity=2 (only 2 possible levels)
- minimum_fill_ratio=1.0 (need both levels before replay)
- ACCEL disabled by default (no meaningful mutations for fixed levels)
"""
...
from jaxued.environments.tmaze import TMaze, make_level_generator, LEVEL_GOAL_LEFT, LEVEL_GOAL_RIGHT
...
def main(config=None, project="egt-pop"):
    ...
    if not config["exploratory_grad_updates"]:
        tags.append("robust")
    tags.append("PLR")
    # If `entity` is None, wandb will use your logged-in default entity.
    run = wandb.init(
        config=config,
        project=project,
    ...
    # Level sampler for T-maze: capacity=2 (only 2 levels), minimum_fill_ratio=1.0
    level_sampler = LevelSampler(
    ...
    def train_step(carry: Tuple[chex.PRNGKey, TrainState], _):
        """
            This is the main training loop. It basically calls either `on_new_levels` or `on_replay_levels` at every step.
            ACCEL (mutation) is disabled for T-maze since there are only 2 fixed levels.
        """
        ...
        # No ACCEL for T-maze - just DR vs Replay decision
    ...
    def eval(rng: chex.PRNGKey, train_state: TrainState):
        """
        This evaluates the current policy on the T-maze evaluation levels.
...
if __name__=="__main__":
    ...
    group.add_argument("--level_buffer_capacity", type=int, default=2)  # Only 2 T-maze levels
    ...
    group.add_argument("--topk_k", type=int, default=2)  # At most 2 levels
    group.add_argument("--minimum_fill_ratio", type=float, default=1.0)  # Need both levels before replay
