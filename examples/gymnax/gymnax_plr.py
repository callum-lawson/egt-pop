...
import wandb
from jaxued.environments.gymnax.pendulum import Pendulum, make_eval_levels_and_names as eval_levels_pendulum, make_level_generator as make_level_generator_pendulum
...
from jaxued.environments.gymnax.cartpole import CartPole, make_eval_levels_and_names as eval_levels_cp, make_level_generator as make_level_generator_cp
from jaxued.environments.gymnax.acrobot import Acrobot, make_eval_levels_and_names as eval_levels_acrobot, make_level_generator as make_level_generator_acrobot
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, x, hidden):
        ...
        hidden, embedding = ResetRNN(nn.GRUCell(features=256))(rnn_in, initial_carry=hidden)
...
class ActorCriticContinuousFF(nn.Module):
    action_dim: Sequence[int]
...
def main(config=None, project="egt-pop"):
    is_continuous = config['env'] in ['pendulum']
    if config['use_rnn']:
        ActorCritic = ActorCriticRNN if not is_continuous else ActorCriticContinuousRNN
    else:
        ActorCritic = ActorCriticFF if not is_continuous else ActorCriticContinuousFF
    ...
    def log_eval(stats, train_state_info):
        ...
        log_dict.update({f"solve_rate/{name}": solve_rate for name, solve_rate in zip(EVAL_LEVEL_NAMES, solve_rates)})
        log_dict.update({"solve_rate/mean": solve_rates.mean()})
        log_dict.update({f"return/{name}": ret for name, ret in zip(EVAL_LEVEL_NAMES, returns)})
    ...
    if config['env'] == 'cartpole':
        env = CartPole()
        sample_random_level = make_level_generator_cp()
        EVAL_LEVELS, EVAL_LEVEL_NAMES = eval_levels_cp()
    elif config['env'] == 'acrobot':
        env = Acrobot()
        sample_random_level = make_level_generator_acrobot()
        EVAL_LEVELS, EVAL_LEVEL_NAMES = eval_levels_acrobot()
    elif config['env'] == 'pendulum':
        env = Pendulum()
        sample_random_level = make_level_generator_pendulum()
        EVAL_LEVELS, EVAL_LEVEL_NAMES = eval_levels_pendulum()
    else:
    ...
    def create_train_state(rng) -> TrainState:
        ...
        network = ActorCritic(env.action_space(env_params).n if not is_continuous else env.action_space(env_params).shape[0])
    ...
    def eval(rng: chex.PRNGKey, train_state: TrainState):
        ...
        levels = EVAL_LEVELS
        num_levels = len(EVAL_LEVEL_NAMES)
    ...
    def eval_checkpoint(og_config):
        ...
        np.savez_compressed(os.path.join(save_loc, 'results.npz'), states=np.asarray(states), cum_rewards=np.asarray(cum_rewards), episode_lengths=np.asarray(episode_lengths), levels=EVAL_LEVEL_NAMES)
    ...
    # Here we run over multiple repeats.
    vmap_train_state     = jax.vmap(create_train_state)
    vmap_train_eval_step = jax.vmap(train_and_eval_step)
    NUM_REPEATS = config["num_repeats"]
    

    train_state = vmap_train_state(jax.random.split(rng_init, NUM_REPEATS))
    runner_state = (rng_train, train_state)
    
    # And run the train_eval_sep function for the specified number of updates
    if config["checkpoint_save_interval"] > 0: checkpoint_manager = setup_checkpointing(config, train_state, env, env_params)
...
if __name__=="__main__":
    ...
    mut_group.add_argument("--num_updates", type=int, default=3_500)
    mut_group.add_argument("--num_env_steps", type=int, default=None)
    group.add_argument("--num_steps", type=int, default=512)
    ...
    # === ENV CONFIG ===
    group.add_argument("--env", type=str, default='cartpole', choices=['cartpole', 'acrobot', 'pendulum'])
    # === DR CONFIG ===
